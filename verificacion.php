<?php
    ...
    if(isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['hash']) && !empty($_GET['hash'])){
        ...
        $select = "SELECT email, hash, activo FROM empleados WHERE email='".$email."' AND hash='".$hash."';" or die(mysqli_error($conn));
        $result = mysqli_query($conn, $select);

        $match  = mysqli_num_rows($result);

        if($match > 0){
            $update = "UPDATE empleados SET activo='1' WHERE email='".$email."' AND hash='".$hash."';" or die(mysqli_error($conn));
